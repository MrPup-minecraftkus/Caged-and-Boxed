...
public class ModBlocks {
    ...
    public static final DeferredBlock<CageBlock> SMALL_CAGE = BLOCKS.register(
            "small_cage",
            id -> new CageBlock(CageSize.SMALL, cageProps().setId(ResourceKey.create(BLOCKS.getRegistryKey(), id)))
    );

    public static final DeferredBlock<CageBlock> MEDIUM_CAGE = BLOCKS.register(
            "medium_cage",
            id -> new CageBlock(CageSize.MEDIUM, cageProps().setId(ResourceKey.create(BLOCKS.getRegistryKey(), id)))
    );

    public static final DeferredBlock<CageBlock> UNIVERSAL_CAGE = BLOCKS.register(
            "universal_cage",
            id -> new CageBlock(CageSize.UNIVERSAL, cageProps().setId(ResourceKey.create(BLOCKS.getRegistryKey(), id)))
    );

    public static final DeferredBlock<BoxBlock> BOX_BLOCK = BLOCKS.register(
            "box_block",
            id -> new BoxBlock(BlockBehaviour.Properties.of()
                    .mapColor(MapColor.WOOD)
                    .strength(1.5f, 3.0f)
                    .sound(SoundType.WOOD)
                    .noOcclusion()
                    .requiresCorrectToolForDrops()
                    .setId(ResourceKey.create(BLOCKS.getRegistryKey(), id)))
    );
}
